...
for (const file of walk(OUT)) {
  ...
  // Idempotent: replace any block a previous build wrote (the root .htaccess
  // is copied from src/ and must keep its own rules above the marker).
  const MARK = "# Generated by scripts/markdown-twins.js";
  const htPath = path.join(dir, ".htaccess");
  const existing = fs.existsSync(htPath) ? fs.readFileSync(htPath, "utf8") : "";
  const keep = existing.includes(MARK) ? existing.slice(0, existing.indexOf(MARK)).replace(/\s+$/, "") : existing.replace(/\s+$/, "");
  fs.writeFileSync(htPath, (keep ? keep + "\n\n" : "") + ht);
}
